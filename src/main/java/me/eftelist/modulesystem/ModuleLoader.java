...
public class ModuleLoader {
    ...
    void loadModule(File file) throws InvalidPluginException {
        ...
        if (!file.exists()) {
            Core.getInstance().getLogger().severe("Trying to load " + file.getPath() + " module. An error occurs : " + "Module file does not exist.");
        } else {
            ModuleDescriptionFile description;
            try {
                description = this.getModuleDescriptionFile(file);
            } catch (InvalidDescriptionException e) {
                Core.getInstance().getLogger().severe("Trying to load " + file.getPath() + " module. An error occurs : " + "Invalid description");
                return;
            }
            ...
            if (moduleClassLoaderMap.get(description.getName()) != null) {
                Core.getInstance().getLogger().severe("Trying to load " + description.getName() + " module. An error occurs : " + "Can't have two module with the same name.");
            } else {
